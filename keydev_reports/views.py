...
from .tasks import get_no_template_report, get_template_report, run_sql_functions
from .providers import (
    ColorfulTableProvider, ProxyModelProvider, TableProvider,
    TemplateProvider, SingleMergedTableProvider, ProxyPDFModelProvider
)
...
def get_pdf_file(request, template, result_content=None):
    ...
    if not os.path.exists(folder):
        os.makedirs(folder)
    file_name = folder + f'{str(request.user.username)}_{datetime.datetime.now().strftime("%d-%m-%Y_%H-%M-%S")}.pdf'
